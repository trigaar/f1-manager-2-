...
// FIX: Exclude 'trait' from the list of skills to progress to prevent type errors in calculations.
const SKILLS_TO_PROGRESS: (keyof Omit<DriverSkills, 'overall' | 'aggressionIndex' | 'incidentProneness' | 'loyalty' | 'potential' | 'trait'>)[] = [
...
export const runDriverProgression = (
    ...
    newRoster.forEach(driver => {
        ...
        if (driver.status === 'Active') {
            ...
            } else { // Plateau or decline phase
                finalChangeFactor = ageModifier + (performanceFactor * 0.5);
            }
        } 
        else if (driver.status === 'Free Agent') {
            averageRating = 50; // Use a neutral-low rating for display purposes
            const ageModifier = getAgeModifier(driver.age);
            const yearsOutFactor = 1 + (Math.min(3, driver.yearsAsFreeAgent || 1) - 1) * 0.25; // 1x for 1yr, 1.25x for 2yr, 1.5x for 3+yr
            
            if (driver.age < 25) { // Stagnation for young Free Agents, with slight decline if out for long
                finalChangeFactor = ((Math.random() - 0.5) * 0.5) - ((yearsOutFactor - 1) * 0.2);
            } else { // Decline for older Free Agents accelerates over time
                finalChangeFactor = (ageModifier * 0.75) * yearsOutFactor;
            }
        } 
        else {
            return; 
        ...
        if (Math.abs(finalChangeFactor) < 0.2) {
             return;
        }

        const oldOverallFromSkills = Math.round(
            SKILLS_TO_PROGRESS.reduce((sum, skill) => sum + driver.driverSkills[skill], 0) / SKILLS_TO_PROGRESS.length
        );

        const progressionEvent: DriverProgressionEvent = {
            driverId: driver.id,
            driverName: driver.name,
            teamName: driver.status === 'Free Agent' ? 'Free Agent' : driver.car.teamName,
            age: driver.age,
            averageRating: parseFloat(averageRating.toFixed(1)),
            changes: [],
            newOverall: driver.driverSkills.overall
        };

        SKILLS_TO_PROGRESS.forEach(skill => {
            const oldValue = driver.driverSkills[skill];
            
            const skillVolatility = (skill === 'qualifyingPace' || skill === 'raceCraft') ? 1.2 : 0.8;
            const randomElement = (Math.random() - 0.5) * 0.5; // Add a small random variance
            
            const change = (finalChangeFactor + randomElement) * skillVolatility;
            
            if (Math.abs(change) < 0.5) return; // Ignore tiny, insignificant changes

            const newValue = Math.round(Math.max(40, Math.min(100, oldValue + change)));

            if (oldValue !== newValue) {
                driver.driverSkills[skill] = newValue;
                progressionEvent.changes.push({
                    skill,
                    oldValue,
                    newValue
                });
        ...
        if (progressionEvent.changes.length > 0) {
            const newOverall = Math.round(
                SKILLS_TO_PROGRESS.reduce((sum, skill) => sum + driver.driverSkills[skill], 0) / SKILLS_TO_PROGRESS.length
            );
            driver.driverSkills.overall = newOverall;
            progressionEvent.newOverall = newOverall;

             progressionEvent.changes.push({
                skill: 'overall',
                oldValue: oldOverallFromSkills,
                newValue: newOverall,
            });

            log.push(progressionEvent);
        }
    });

    log.sort((a,b) => {
        const changeA = a.changes.find(c => c.skill === 'overall')!.newValue - a.changes.find(c => c.skill === 'overall')!.oldValue;
        const changeB = b.changes.find(c => c.skill === 'overall')!.newValue - b.changes.find(c => c.skill === 'overall')!.oldValue;
        return changeB - changeA;
    });

    return { newRoster, log };
}