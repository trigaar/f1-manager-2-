...
const calculateNextStates = (
    ...
    for (const driver of nextDrivers) {
        ...
        if (driver.raceStatus === 'Crashed' || driver.raceStatus === 'DNF' || nextRaceState.flag === RaceFlag.Red) continue;
...
const App: React.FC = () => {
  ...
  const handleStartNewSeason = () => {
    ...
    if (season === 2025 && !personnel.some(p => p.teamName === 'Cadillac Racing')) {
        ...
        const cadillacSignings = freeAgents.slice(0, 2);

        cadillacSignings.forEach(signing => {
            const driverIndex = updatedRosterWithHistory.findIndex(d => d.id === signing.id);
            if (driverIndex !== -1) {
                const driverToUpdate = updatedRosterWithHistory[driverIndex];
                driverToUpdate.status = 'Active';
                driverToUpdate.car = cadillacCar;
                driverToUpdate.contractExpiresIn = 2;
                driverToUpdate.careerHistory[season + 1] = 'Cadillac Racing';
  ...
  }, [gamePhase, raceState.lap, raceState.totalLaps, runSimulationLap]);
